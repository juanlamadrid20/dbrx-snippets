...
def gold_features_customer_engagement():
    return (
...
def gold_features_customer_behavior():
    ...
    return (
        ...
        .withColumn("device_total_sessions", 
        ...
        .withColumn("mobile_preference_score",
                    when(col("device_total_sessions") > 0,
                         round(coalesce(col("mobile_sessions"), lit(0)) / col("device_total_sessions"), 3))
                    .otherwise(0.0))
        .withColumn("desktop_preference_score",
                    when(col("device_total_sessions") > 0,
                         round(coalesce(col("desktop_sessions"), lit(0)) / col("device_total_sessions"), 3))
                    .otherwise(0.0))
...
def ml_feature_table():
    engagement_features = dlt.read("gold_features_customer_engagement")
    behavior_features = dlt.read("gold_features_customer_behavior") 
    ...
    return (
        ...
        .select(
            ...
            # Behavioral features  
            "desktop_sessions", "mobile_sessions", "tablet_sessions", "device_total_sessions",
